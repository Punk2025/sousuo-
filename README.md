...
2. **推荐用终端**（避免 Telegram/网盘传过去后提示「已损坏」）：

```bash
cd ~/Desktop/搜索网   # 改成你的路径
bash mac/终端一键.sh
```

也可双击 `Mac一键安装.command`；若提示无法打开：右键 → **打开**，或看根目录 `终端说明.txt`。

3. 按提示操作（首次会装 Homebrew → Python → 依赖 → Chromium，可能要输入 Mac 密码）
4. 安装完成后也可用桌面 **`SearchPipe.command`** 启动（仍被拦就继续用 `bash mac/终端一键.sh`）

控制面板地址：http://127.0.0.1:8878/admin/  
停止服务：在终端窗口按 `Ctrl+C`。

> 完整终端指令见 [`终端说明.txt`](./终端说明.txt)。
...
4. [前端分流与 iframe 导航壳](./docs/前端分流与iframe导航壳.md)  
5. [搜索管线 · SERP 报表与打标](./docs/搜索管线-SERP报表与打标.md)  
6. [相关开源与资料](./docs/相关开源与资料.md)  
